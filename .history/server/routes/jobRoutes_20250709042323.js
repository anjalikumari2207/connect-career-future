...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    // ✅ Validate transaction on Solana Devnet
    ...
    const isPaymentToAdmin = tx.transaction.message.instructions.some(instr => {
      const parsed = instr.parsed;
      return parsed?.info?.destination === adminWallet;
    });

    if (!isPaymentToAdmin) {
      return res.status(400).json({ msg: "Transaction not sent to admin wallet." });
    }

    // ✅ Save job with txHash
